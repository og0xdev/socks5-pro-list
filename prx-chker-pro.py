...
def main():
    if not os.path.exists('huge_socks5.txt') or input("Fetch fresh proxies? (y/n): ").lower() == 'y':
        proxies_list = fetch_all_proxies('misc.txt')
    else:
        proxies_list = [line.strip() for line in open('huge_socks5.txt') if is_valid_proxy_format(line.strip())]
    
    if not proxies_list:
        print("[WRN] No valid proxies found!")
        return
    
    target_host = input("Target host [httpbin.org]: ").strip() or "httpbin.org"
    target_port = int(input("Target port [80]: ").strip() or "80")
    max_workers = int(input("Threads [100]: ").strip() or "100")
    max_latency = int(input("Max latency (ms) [800]: ").strip() or "800")
    
    print(f"\n[INFO] Testing {len(proxies_list)} valid proxies with {max_workers} threads...")
    good_proxies = []
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(SOCKS5Checker(proxy, (target_host, target_port)).check): proxy for proxy in proxies_list}
    ...
    print(f"\n[INFO] Results: {len(good_proxies)}/{len(proxies_list)} proxies under {max_latency}ms")
    
    if good_proxies:
        good_proxies.sort(key=lambda x: x['latency'])
        top_10 = good_proxies[:10]
        
        print("\n[INFO] Running 4 bandwidth tests each on top 10 proxies (threaded)...")
        ...
        print("\n[HIT] Top 10 Proxies by Average Bandwidth (4 tests each):")
        print("=" * 65)
        for i, result in enumerate(bandwidth_results, 1):
            test_str = "/".join(f"{s:.1f}" for s in result['tests'])
            print(f"{i:2d}. {result['proxy']} - {result['bandwidth_mbps']:.1f} Mbps ({test_str}) - {result['latency']:.0f}ms")
        
        with open('fast_proxies.txt', 'w') as f:
            for result in bandwidth_results:
                test_str = "/".join(f"{s:.1f}" for s in result['tests'])
                f.write(f"{result['proxy']} | {result['bandwidth_mbps']:.1f} Mbps ({test_str}) | {result['latency']:.0f}ms\n")
